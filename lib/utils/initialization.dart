...
import 'package:quran/data/models/chaptermodels.dart' hide RevelationType;
import 'package:quran/data/models/jsonmodeltranslation.dart';
import 'package:quran/data/models/qurantextformat.dart';
import 'package:quran/data/models/yearlyprayertiming.dart';
import 'package:quran/utils/reciters/reciters.dart';
...
class Initialization {
  static checkPrayers() async {
    ...
    if (UC.hive.get(kAdhanNotificationSet) != dt.month) {
      ...
      UserPrayerPreference userPrayerPreference = UserPrayerPreference.fromJson(json.decode(userPreference));
      String? yearlyPrayerTimings = UC.hive.get(kYearlyPrayerTimings);
      if (yearlyPrayerTimings == null) {
        yearlyPrayerTimings ??= await compute(getYearlyPrayerTimings, userPrayerPreference);
        UC.hive.put(kYearlyPrayerTimings, yearlyPrayerTimings);
      }
      if (yearlyPrayerTimings != null &&
          YearlyPrayerTiming.fromJson(json.decode(yearlyPrayerTimings)).data.months.first.first.date.gregorian.year !=
              dt.year.toString()) {
        String? yearlyPrayerTimings = await compute(getYearlyPrayerTimings, userPrayerPreference);
        UC.hive.put(kYearlyPrayerTimings, yearlyPrayerTimings);
        if (yearlyPrayerTimings != null) {
  ...
  initializePrayers() async {
    ...
    if (location != null) {
      ...
      UserPrayerPreference userPrayerPreference =
          UserPrayerPreference(country: locationApi.country, city: locationApi.city);
      ...
      throw Exception("Internet is required for the first time");
    }
  }
    ...
    for (EachTranslation translation in totalTranslations) {
      ...
      isarTranslations.add(
        TextTranslation(
          ...
          format: translation.format!.toString().split('Format.')[1].toLowerCase(),
          type: translation.type!.toString().split('Type.')[1].toLowerCase(),
          direction: translation.direction!.toString().split('Direction.')[1].toLowerCase(),
  ...
  Future<bool> initializeAyahs(bool _) async {
    ...
    for (SurahsReference surah in surahs) {
      newSurahs.add(Surah(
        ...
        revelationType: surah.revelationType == RevelationType.MECCAN ? "Meccan" : "Medinan",
      ));
    }
