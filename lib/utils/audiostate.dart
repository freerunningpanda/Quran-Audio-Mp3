...
class AudioManager with ChangeNotifier {
  ...
  Future<Duration?> reciteAyah({required Ayah ayah}) async {
    try {
      ...
      String link = "https://cdn.islamic.network/quran/audio/192/ar.abdulbasitmurattal/${ayah.number}.mp3";
      Duration? duration = await player.setAudioSource(
        LockCachingAudioSource(
          Uri.parse(link),
          cacheFile: File('${UC.uv.appDirectoryPath}/ar.abdulbasitmurattal/${ayah.chapterNo}_${ayah.numberInSurah}'),
          tag: MediaItem(
  ...
  reciteSurah(List<AudioSource> chapters, Surah surah, String album, {bool startReciting = true}) async {
    try {
      ...
      player.setAudioSource(
        ...
        initialPosition: Duration(seconds: surah.currentDuration ?? 0),
      );
  ...
  initializeOldRecitingSurahsIfExist(Surah surah, {bool startReciting = false}) async {
  ...
  AudioManager() {
    currentRecitingSurah = UC.isar.surahs.filter().lastRecitedIsNotNull().sortByLastRecitedDesc().findFirstSync();
    if (currentRecitingSurah != null) {
      currentDuration = Duration(seconds: currentRecitingSurah?.currentDuration ?? 0);
      totalDuration = Duration(seconds: currentRecitingSurah?.totalDuration ?? 1);
    ...
    player.playbackEventStream.listen((playBack) async {
      ...
      if (currentIndex == currentRecitingSurah?.number) {
        currentRecitingSurah?.lastRecited = DateTime.now().millisecondsSinceEpoch;
