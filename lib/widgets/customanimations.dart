...
class MyCustomAnimation extends StatefulWidget {
  ...
  const MyCustomAnimation({Key? key, this.duration = const Duration(seconds: 3)}) : super(key: key);
...
class CountDownAnimation extends StatefulWidget {
  ...
  const CountDownAnimation(
      {Key? key, required this.child, this.duration = const Duration(seconds: 4), this.color = Colors.green})
      : super(key: key);
...
class AyahRecitingWidget extends StatefulWidget {
  ...
  const AyahRecitingWidget({Key? key, this.text = 'Ayah Reciting', this.duration, required this.textDirection})
      : super(key: key);
...
class _AyahRecitingWidgetState extends State<AyahRecitingWidget> with SingleTickerProviderStateMixin {
  late final AnimationController _controller;
  double value = 0.0;
  @override
  void initState() {
    super.initState();
    _controller = AnimationController(duration: widget.duration ?? const Duration(minutes: 1), vsync: this);
    _controller.value = 1.0;
    _controller.reverse();
    _controller.addListener(() {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: Column(
        ...
        children: [
          Align(
            alignment: widget.textDirection == TextDirection.ltr ? Alignment.topLeft : Alignment.topRight,
            child: MarqueeWidget(
          ...
          LinearProgressIndicator(
            ...
            color: Colors.green,
            valueColor: widget.duration == null ? const AlwaysStoppedAnimation(Colors.yellow) : null,
          )
        ],
      ),
    );
