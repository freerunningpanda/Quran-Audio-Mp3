...
final searchProvider = riverpod.ChangeNotifierProvider.autoDispose((_) => SearchController());
    ...
    if (qr.length == 1) {
      ...
      compute(finAyahsBySentence, str).then((response) {
        if (response[0].isNotEmpty) {
          ...
          status = AppStrings.nothingFound;
          foundArabicAyahs.clear();
          foundTranslationAyahs.clear();
        }
...
class Search extends riverpod.ConsumerWidget {
          ...
          appBar: AppBar(
              ...
              title: Container(
                ...
                child: CupertinoTextField(
                  ...
                  suffix: InkWell(
                      onTap: () {
                        sP.searchByNameOrNumber(textEditingController.value.text);
                      },
                                  ...
                                  child: SlideAnimation(
                                    ...
                                    child: FadeInAnimation(
                                      child: EachAyahWidget(
                                        no: sP.foundArabicAyahs[index].numberInSurah,
                                        copyText: () {
                                        ...
                                        share: () async {
                                          Navigator.push(context, MaterialPageRoute(builder: (context) {
                                            return StatusMaker(ayahNumber: sP.foundTranslationAyahs[index].number);
                                          }));
                                        },
                                        chapterNo: sP.foundArabicAyahs[index].chapterNo,
                                        arabic: sP.foundArabicAyahs[index].text,
                                        translation: sP.foundTranslationAyahs[index].text,
                                        textDirection: sP.foundTranslationAyahs[index].direction == "ltr"
                                            ? TextDirection.ltr
                                            : TextDirection.rtl,
                                        textDirectionString: sP.foundTranslationAyahs[index].direction,
                                        sajda: sP.foundTranslationAyahs[index].sajda ?? '',
                                        bookmarked: sP.foundTranslationAyahs[index].bookmarked ?? false,
                                        bookmark: () {
                                          sP.bookmarkAyah(sP.foundTranslationAyahs[index]);
                                        },
                                        audio: () async {
                                          sP.reciteAyah(
                                              context: context, ref: ref, ayah: sP.foundTranslationAyahs[index]);
                                        },
                                      ),
                                    ),
                                  ),
